...
def budgeted_gate_strict(
    ...
    mats_test = [base_test] + [np.asarray(P, float) for (_, P) in experts_test]
    if len(mats_test) != E:
        raise ValueError(f"Mismatch: cluster_nll expects E={E}, but got {len(mats_test)} test matrices.")
    ...
    base_nll = cluster_nll[ct_test, 0]
    delta_improve = base_nll - best_nll  # + => better than base

    base_conf = base_test.max(axis=1)
    ...
    cand = (best_e != 0) & (delta_improve > float(margin_min_nll))
    if float(delta_conf) > 0.0:
        cand &= (conf_margin > float(delta_conf))

    cand_idx = np.where(cand)[0]
    if cand_idx.size == 0:
        return base_test.copy()

    # priority: expected gain x base uncertainty
    ...
    P_fused = base_test.copy()
    for e in range(1, E):
        take = chosen[best_e[chosen] == e]
        if take.size:
            P_fused[take] = mats_test[e][take]

    # HARD GUARANTEE: return is numeric (N,C)
    return np.asarray(P_fused, dtype=float)
